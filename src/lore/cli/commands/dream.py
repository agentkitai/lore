...
def _build_prompt(
    *,
    ...
    return (
        ...
        "   same thing, call mcp__lore__consolidate(memory_ids=[...]).\n"
        "2. RESOLVE contradictions: query mcp__lore__conflicts; pick the\n"
        "   one supported by newer corrections; downvote/forget the loser.\n"
        "   When you see a correction in recent signal (e.g. \"actually X\n"
        "   is Y now\", \"we changed our approach\"), prefer\n"
        "   mcp__lore__supersede(old_memory_id, new_memory_id, reason)\n"
        "   over mcp__lore__forget(old_memory_id). Supersede preserves the\n"
        "   audit trail; forget destroys it.\n"
        ...
        "   >3x and importance > 0.7, call mcp__lore__remember(\n"
