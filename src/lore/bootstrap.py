...
class BootstrapRunner:
    ...
    def run_all(self) -> List[CheckResult]:
        ...
        if not self.skip_docker:
            ...
            results.append(self.check_docker_compose())
    ...
    def check_env_file(self) -> CheckResult:
        """Check .env file exists; create with sensible defaults if --fix."""
        import os
        ...
        if os.path.exists(env_path):
            return CheckResult(
                name="env_file",
                status="ok",
                message=f".env exists at {env_path}",
            )
        if self.fix:
            ...
            return CheckResult(
                name="env_file",
                status="ok",
                message=f"Created .env with defaults at {env_path}",
            )
        return CheckResult(
            name="env_file",
            status="fail",
            message=".env not found in current directory",
            fix_hint="Run with --fix to create a default .env, or create one manually",
        )
    ...
    def check_docker_compose(self) -> CheckResult:
        """Check docker-compose.yml exists; offer to start containers if --fix."""
        import os
        ...
        if not os.path.exists(compose_file):
            return CheckResult(
                ...
                fix_hint="Run from the project root that contains docker-compose.yml",
            )
        if self.fix:
            try:
                print("  Starting Docker containers (docker compose up -d)...")
                result = subprocess.run(
                    ["docker", "compose", "up", "-d"],
                ...
                return CheckResult(
                    name="docker_compose",
                    status="fail",
                    message=f"docker compose up -d failed: {result.stderr.strip()[:200]}",
                    fix_hint="Check docker-compose.yml and try: docker compose up -d",
                )
            except (subprocess.TimeoutExpired, FileNotFoundError) as exc:
                return CheckResult(
                    name="docker_compose",
                    status="fail",
                    message=f"docker compose up -d failed: {exc}",
                    fix_hint="Ensure Docker is running and try: docker compose up -d",
                )
        return CheckResult(
            name="docker_compose",
            status="ok",
            message=f"docker-compose.yml found at {compose_file}",
            fix_hint="Run with --fix to start containers automatically",
        )
...
_DEFAULT_ENV = """\
# Lore environment config — generated by `lore bootstrap --fix`
DATABASE_URL=postgresql://lore:lore@localhost:5432/lore
LORE_STORE=local
LORE_PROJECT=default
# LORE_API_KEY=
# LORE_LLM_PROVIDER=openai
# LORE_LLM_API_KEY=
# LORE_ENRICHMENT_ENABLED=false
"""


def _create_default_env(path: str) -> None:
...
def format_results(results: List[CheckResult], verbose: bool = False) -> str:
    ...
    if fail_count == 0:
        ...
        lines.append("")
        lines.append("  Setup complete! Try: lore remember 'Hello world'")
    else:
