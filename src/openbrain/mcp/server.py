"""Open Brain MCP server — exposes 5 tools over stdio transport.

Tools: remember, recall, forget, list, stats

Configure via environment variables:
  OPENBRAIN_STORE   — "local" (default) or "remote"
  OPENBRAIN_PROJECT — default project scope
  OPENBRAIN_API_URL — required when OPENBRAIN_STORE=remote
  OPENBRAIN_API_KEY — required when OPENBRAIN_STORE=remote
  OPENBRAIN_DB_PATH — SQLite path (local mode only)
  OPENBRAIN_MODEL_DIR — embedding model cache directory
"""
...
def _get_store():
    ...
    store_type = os.environ.get("OPENBRAIN_STORE", "local").lower()
    _default_project = os.environ.get("OPENBRAIN_PROJECT") or None

    if store_type == "remote":
        api_url = os.environ.get("OPENBRAIN_API_URL")
        api_key = os.environ.get("OPENBRAIN_API_KEY")
        if not api_url or not api_key:
            raise RuntimeError(
                "OPENBRAIN_API_URL and OPENBRAIN_API_KEY must be set when OPENBRAIN_STORE=remote"
            )
        from openbrain.store.remote import RemoteStore
        _store = RemoteStore(api_url=api_url, api_key=api_key)
    else:
        db_path = os.environ.get(
            "OPENBRAIN_DB_PATH",
            os.path.join(os.path.expanduser("~"), ".openbrain", "default.db"),
        )
...
def _get_embedder():
    ...
    model_dir = os.environ.get("OPENBRAIN_MODEL_DIR")
...
def _is_remote() -> bool:
    return os.environ.get("OPENBRAIN_STORE", "local").lower() == "remote"
...
mcp = FastMCP(
    name="openbrain",
    instructions=(
        "Open Brain is a persistent memory system for AI. Use it to save "
...
def remember(
    ...
    try:
        ...
        if _is_remote():
            # Remote mode: the server handles embedding
...
def recall(
    ...
    """Search Open Brain memory for relevant memories."""
    try:
        store = _get_store()
...
def forget(
    ...
    """Delete memories from Open Brain."""
    try:
        store = _get_store()
