...
# ── 2. PostgreSQL ──────────────────────────────────────────────────
PG_VERSION=""
# Prefer a version that is already running
for v in 17 16 15; do
  if brew services list 2>/dev/null | grep "postgresql@$v" | grep -q started; then
    PG_VERSION=$v
    break
  fi
done
...
if [ -z "$PG_VERSION" ]; then
  for v in 17 16 15; do
    if brew list --versions "postgresql@$v" &>/dev/null; then
      PG_VERSION=$v
      break
    fi
  done
...
# Create a dedicated venv so packages match the Python version
LORE_VENV="$HOME/.lore/venv"
if [ ! -d "$LORE_VENV" ]; then
  echo "📦 Creating Lore virtualenv at $LORE_VENV..."
  $PY_BIN -m venv "$LORE_VENV"
fi
PY_BIN="$LORE_VENV/bin/python"
...
assert v >= Version('0.4.0'), f'Got {v}, need >= 0.4.0'
" 2>&1; then
  echo "✅ Lore SDK v$LORE_VERSION installed"
else
  echo "❌ lore-sdk version $LORE_VERSION is too old (need >= 0.4.0)"
  echo "   This usually means pip resolved an old version for your Python."
  echo "   Debug: $PIP_CMD install --upgrade --verbose 'lore-sdk[server,enrichment]'"
  exit 1
fi

# ── Download missing migration files ──────────────────────────────
LORE_MIGRATIONS_DIR=$($PY_BIN -c "import lore, pathlib; print(pathlib.Path(lore.__file__).parent / 'migrations')" 2>&1)
LORE_GIT_REF="b3948c5"
echo "  Checking migrations in $LORE_MIGRATIONS_DIR..."
for mig in 001_initial.sql 004_sharing.sql 005_oidc_and_rbac.sql 006_importance_scoring.sql 008_conversation_jobs.sql 009_rename_to_memories.sql 010_retrieval_analytics.sql; do
  if [ ! -f "$LORE_MIGRATIONS_DIR/$mig" ]; then
    echo "  📥 Downloading $mig..."
    curl -sSL "https://raw.githubusercontent.com/agentkitai/lore/$LORE_GIT_REF/migrations/$mig" -o "$LORE_MIGRATIONS_DIR/$mig"
  fi
done
# Fix migration ordering: 007 depends on memories table created in 009
if [ -f "$LORE_MIGRATIONS_DIR/007_knowledge_graph.sql" ] && [ ! -f "$LORE_MIGRATIONS_DIR/011_knowledge_graph.sql" ]; then
  mv "$LORE_MIGRATIONS_DIR/007_knowledge_graph.sql" "$LORE_MIGRATIONS_DIR/011_knowledge_graph.sql"
fi
...
if [ "$USE_MAX_PROXY" = true ]; then
  ...
  MAX_PROXY_BIN="$NPM_BIN_DIR/claude-max-api"

  if [ ! -f "$MAX_PROXY_BIN" ]; then
    echo "❌ claude-max-api binary not found at $MAX_PROXY_BIN"
    exit 1
  fi
...
LORE_STORE=postgres
DATABASE_URL=postgresql://localhost/lore
MIGRATIONS_DIR=$($PY_BIN -c "import lore; import pathlib; print(pathlib.Path(lore.__file__).parent / 'migrations')")
...
# ── 10b. Schema fix: context column must be nullable ─────────────
psql lore -c "ALTER TABLE memories ALTER COLUMN context DROP NOT NULL;" 2>/dev/null || true

# ── 10c. Initialize org and API key ──────────────────────────────
echo "📦 Initializing organization..."
ORG_INIT=$(curl -sf -X POST "http://localhost:$LORE_PORT/v1/org/init" \
  -H "Content-Type: application/json" \
  -d '{"name": "default"}' 2>&1)

if echo "$ORG_INIT" | $PY_BIN -c "import sys,json; d=json.load(sys.stdin); print(d['api_key'])" > /dev/null 2>&1; then
  LORE_API_KEY=$( echo "$ORG_INIT" | $PY_BIN -c "import sys,json; d=json.load(sys.stdin); print(d['api_key'])" )
  echo "✅ Organization created with API key: ${LORE_API_KEY:0:16}..."
  # Update the env file with the real API key
  sed -i '' "s|^LORE_API_KEY=.*|LORE_API_KEY=${LORE_API_KEY}|" "$LORE_ENV"
else
  echo "⚠️  Org already exists or init failed (this is OK on re-install)"
  echo "  Response: $ORG_INIT"
fi
