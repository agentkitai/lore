...
- **Contradiction detection is default-on (when an LLM is configured) and now soft-supersedes.** Write-time contradiction detection was off-by-default and flag-only; it is now on by default whenever `OPENAI_API_KEY` is set (override with `LORE_CONTRADICTION_DETECTION`), and on a high-confidence contradiction it **soft-supersedes** the older contradicted memory (last-write-wins) in addition to flagging the new one. Superseded memories are score-suppressed in recall (×0.1), not deleted — reversible and audited (`memory_supersessions`). Only your own / unowned memories are auto-superseded; cross-agent conflicts stay flag-only. Tunable via `LORE_CONTRADICTION_SUPERSEDE` (default `true`) and `LORE_CONTRADICTION_SUPERSEDE_MIN_CONFIDENCE` (default `0.75`). This aligns Lore's staleness handling with the field norm (Zep/Graphiti invalidation, Mem0 write-time reconciliation): retain-and-supersede, not hard-delete.
...
### Fixed
- **Web UI (`/ui`) now works against an authed (remote / docker-stack) server.** The bundled graph UI made all API calls with no `Authorization` header, so on a Postgres-backed server that enforces keys every fetch returned 401 ("Failed to load graph") and nothing rendered. The UI now sends a Bearer key, captured once at load from `?key=…` (then persisted to `localStorage` and stripped from the URL so it isn't left in history) or a prior stored value. Open `http://<host>/ui/?key=<LORE_API_KEY>` once; reloads stay authed. Solo/local mode is unaffected (no key → header omitted).
...
### Added
- **Declarative root-key provisioning (`LORE_API_KEY` seeding).** A Postgres-backed server now seeds an org + root API key from the `LORE_API_KEY` env var on first boot when the DB is empty, so a docker-compose / k8s deploy is usable immediately — previously the only way to mint the first key was the one-time `POST /v1/org/init` (random key, returned once), which a static manifest can't capture. No-op when `LORE_API_KEY` is unset or any org already exists (never overwrites a real deployment). Solo SQLite mode is unaffected.

### Added
- **Phase 6G — claude-mem parity (timeline drill-down + scope + UserPromptSubmit/SessionEnd hooks).** Closes three architectural gaps surfaced by the [claude-mem talk](https://youtu.be/KM2qHN3cMnU). New `memories.scope` column (`'project' | 'global'`) gates retrieval — `recall` and `search` now filter to `(scope='global' OR same project)` by default, with `scope='all'` opt-out for cross-project searches. Manual `remember(...)` defaults `scope` by type (`lesson`/`preference`/`pattern`/`convention` → `global`; everything else → `project`); the auto-capture subagent now LLM-classifies `scope` per emitted observation. New `resolve_project()` helper auto-derives the lore project from git context (remote URL → common-dir basename fallback), so memories captured in different worktrees of the same repo correctly share scope. New `mcp__lore__timeline(anchor_id, limit, direction, max_hours)` MCP tool (and `GET /v1/timeline` endpoint) sits between `search` and `get_memories` in the progressive-disclosure flow — returns chronologically adjacent events around an anchor (~60 tokens/entry, ±10 entries hard-capped at ±2h, same-project only) so the agent can establish causality before paying for full content. New `UserPromptSubmit` hook (`hooks/lore-capture-prompt.sh`) strips `<private>...</private>` blocks fail-closed and appends `{kind:"prompt", text}` entries to the existing per-session buffer; the extraction subagent now reads them as user-intent context and emits one consolidated `meta.kind="intent"` observation per batch when prompts are present. New `SessionEnd` hook (`hooks/lore-capture-end.sh`) runs a foreground final flush, then `lore session-finalize` emits one `meta.kind="summary"` observation summarizing the session — idempotent via a `sealed` marker. Migration `024_scope_and_kind.sql` adds the column + two indices (`idx_memories_scope_project`, `idx_memories_project_session`) and backfills existing `lesson`/`preference`/`pattern`/`convention` rows to `scope='global'`. Tunables: `LORE_PROMPT_MAX_BYTES` (default 8 KB), `LORE_PROJECT_OVERRIDE` (skip git resolution). **Behavior change:** unscoped recall queries with no current project now restrict to `scope='global'` rows only — SDK callers who relied on cross-project bleed-through must either set a `project` or pass `scope_mode='all'`. Spec: `docs/superpowers/specs/2026-05-07-lore-claude-mem-parity-design.md`.
- **Lazy server (`lore serve --idle-timeout`) + auto-start hooks.** `lore serve` grows an `--idle-timeout N` flag (default 0 = current behavior, fully backward compatible). When N > 0, the FastAPI app installs a `LastRequestTracker` middleware that updates a monotonic-clock sentinel on every request (including `/health`, so hook health-pings count as keep-alive), and the lifespan spawns an `idle_watcher_loop` task that wakes every 60s and calls `os._exit(0)` once the elapsed time crosses the limit — logging "idle-timeout exit" first. The CLI flag falls back to `LORE_IDLE_TIMEOUT` env var so the four hook templates in `src/lore/setup.py` can spawn the server with a uniform 1h default. New shared bash preamble `_lore_ensure_server` is rendered into all four Claude Code hooks (retrieval, capture-tool, capture-stop, dream-trigger): each polls `$LORE_API_URL/health` first and short-circuits if the server is up, otherwise spawns `nohup lore serve --port 8765 --idle-timeout "${LORE_IDLE_TIMEOUT:-3600}"` and waits up to ~3s for the bind. Tunables: `LORE_IDLE_TIMEOUT` (idle window, default 3600s), `LORE_NO_AUTOSTART=true` (skip the spawn entirely; for users who want manual control). The retrieval hook is python-based so it invokes the same bash via `subprocess.run`, keeping a single source of truth. Hooks fail-open — if the server can't come up the hook still exits 0 and Claude Code is never blocked.
- **Phase 6A — Lore auto-capture pipeline.** Memory now grows on its own as the user works in Claude Code, no `remember()` calls required. New `lore capture-extract` CLI subcommand (`src/lore/cli/commands/capture.py`) is the subagent entry point: reads the per-session buffer (`~/.lore/sessions/<sid>/buffer.jsonl`), slices unprocessed events past the cursor, builds an extraction prompt with transcript context + recent-memory titles, and fires `claude -p` as a fully detached subprocess. Two new bash hook templates in `src/lore/setup.py` — `LORE_CAPTURE_TOOL_HOOK_SCRIPT` (PostToolUse: appends to buffer, spawns extract every `LORE_CAPTURE_N` events; default 10) and `LORE_CAPTURE_STOP_HOOK_SCRIPT` (Stop: flushes the trailing batch unconditionally) — installed alongside the existing UserPromptSubmit retrieval hook by `setup_claude_code()`. Hooks are fail-open (always exit 0), use `flock` for concurrency control, and skip `mcp__lore__*` calls to prevent recursion. Tunables: `LORE_AUTO_SAVE` master switch, `LORE_CAPTURE_N`, `LORE_CAPTURE_SKIP`, `LORE_CAPTURE_TRANSCRIPT_TURNS`, `LORE_CAPTURE_RECENT_MEMORIES`. Spec: `docs/superpowers/specs/2026-05-07-lore-auto-capture-design.md`.
...
- **Phase 3J: bootstrap + typed-exception parity + serve defaults.** Final Phase 3 sub-phase makes a fresh `pip install lore-sdk[solo] && lore serve` work without any config:
  ...
  * **Default `database_url`.** `Settings.from_env()` now resolves `LORE_DATABASE_URL` → legacy `DATABASE_URL` → `sqlite:///~/.lore/lore.db`. Backwards-compatible with existing Postgres deployments that already set `DATABASE_URL`; new solo installs land on the file-backed default.
...
- **Phase 4A: AsyncLore embedded library — skeleton + foundational methods.** First slice of the spec's "Component 4" (`lore.AsyncLore`). New `src/lore/async_lore.py` exports a single class that opens a `Store` via `make_store(database_url)`, validates / auto-resolves the bound `org_id`, and exposes a thin async surface that delegates to the same `services/*` layer the HTTP routes use. Phase 4A ships the lifecycle plumbing plus 5 foundational methods — `remember`, `recall`, `get`, `forget`, `list_memories`. Each is a few lines: build typed params, await the matching service function, return the dataclass. Phase 4B will extend the surface to the full ~30-method MCP/SDK parity; Phase 4C will start the background workers (SLO, retention, alerting, ingest) inside `__aenter__` / `__aexit__`.
  * **Lifecycle.** `async with AsyncLore(database_url, *, workspace="solo", api_key=None, embed=None, org_id=None) as lore:` — the context-manager entry opens the Store and (for `sqlite:///:memory:` URLs) re-runs the Phase-3J bootstrap with `force_for_memory=True` so the embedded API always has a real `orgs(id='solo')` row to attach to. Exit closes the Store (await-tolerant for sync `close()`s on the legacy `MemoryStore` shape).
...
- **Phase 5: `lore migrate` command — bidirectional data migration between any two Store backends.** Final phase of the SQLite solo-mode design: with Postgres and SQLite both speaking the full Store protocol after Phase 3, and `AsyncLore` letting consumers run embedded after Phase 4, Phase 5 closes the loop with a one-shot `lore migrate --from <url> --to <url>` that streams every table in dependency order while preserving server-generated IDs and timestamps bit-exact.
  ...
  * **Schema-version compatibility check.** Before any copy starts, both endpoints' `schema_migrations` tables are read (when present) and compared. A mismatch — e.g. target missing migration `019` — refuses with a clear list of missing versions and exit code 3, before any rows touch the target. Postgres' `run_migrations` doesn't currently track applied versions, so when one side lacks the table the check is skipped and per-table existence guards take over.
  ...
  * **Migration order.** 26 tables in the order set by FK dependencies — `orgs → users → workspaces → workspace_members → api_keys → retrieval_profiles → memories → entities → entity_mentions → relationships → rejected_patterns → review_decisions → retention_policies → snapshot_metadata → restore_drill_results → slo_definitions → slo_alerts → sharing_config → agent_sharing_config → deny_list_rules → sharing_audit → audit_log → retrieval_events → recommendation_config → recommendation_feedback → conversation_jobs`.
  * **Tests.** New `tests/migrate/test_migrate_round_trip.py` (6 tests): pg→sqlite round-trip with row-count + ID preservation against the canonical test DB; sqlite→sqlite migrate + `--continue` resume verifying no duplicates; `--dry-run` writes nothing; `--re-embed` regenerates vec0 floats via a mock embedder; schema-mismatch refuses upfront with exit code 3; embedding-dim mismatch without `--re-embed` refuses with exit code 4. Tests redirect the state-file path through a `Path.home` monkeypatch fixture so they don't pollute the real `~/.lore/migrate-state.json`.
