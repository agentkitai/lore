-- Migration 007: Knowledge Graph Layer (SQLite translation)
-- Adds entities, relationships, and entity_mentions tables.
--
-- Translation notes:
--   * JSONB → TEXT, TIMESTAMPTZ → TEXT.
--   * Partial unique indexes (`WHERE valid_until IS NULL`) are supported by SQLite.
--   * entity_mentions.memory_id references the lessons table at this point
--     in migration history (renamed to memories in 009). FK preserved.
